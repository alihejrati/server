...
var db_log = db.model('log', new mongoose.Schema({"data":{"type":"Object","required":true},"level":{"type":"Object","required":true},"sort":{"type":"Number","default":1},"tag":{"type":["String"],"default":[]},"flag":{"type":"Object","default":{"hide":false,"delete":false}}}, {timestamps: true}));
...
(async () => {
    const document = await db_log.findOne({}).sort({}).select({});
    const value = JSON.parse(JSON.stringify(document));
    const seed = [];
    if (!value) {
        const database = 'db';
        const collection  = 'log';
        const documents = await db_log.insertMany(seed, { ordered: false });
        documents.length > 0 ? console.debug('seeder: (mongodb, ' + database + ', ' + collection + ', ' + documents.length + ')') : null;
    } 
})();
